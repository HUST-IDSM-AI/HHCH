...
def computeInstanceLoss(p_1, p_2, option, proto_index, index):
    if option.HIC:
        ...
        return instance_contrastive_loss
...
def computeProtoLoss(p_1, p_2, option, proto_corresponding, proto_index, index, cluster_results):
    if option.HPC:
...
def train_step(model, optimizer, train_loader, cluster_results, epoch, state, option=None):
    ...
    if not option.IC:
        ...
        proto_corresponding, proto_index = None, None
    ...
    for i, ((feature_1, feature_2, feature_origin), target, index) in enumerate(train_loader):
        ...
        #############quantization loss###########
        quantization_loss = computeQuantLoss(h_1, h_2)
        ###########contrastive loss###############
        instance_contrastive_loss = computeInstanceLoss(p_1, p_2, option, proto_index, index)
        proto_contrastive_loss = computeProtoLoss(
            p_1, p_2, option, proto_corresponding, proto_index, index, cluster_results)
